...
export function Budgets() {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="w-full bg-muted rounded-full h-4 overflow-hidden mb-2">
          <div
            className={`h-full transition-all rounded-full ${
              getStatus(totalPercentage) === 'exceeded'
                ? 'bg-destructive'
                : getStatus(totalPercentage) === 'warning'
                ? 'bg-warning'
                : 'bg-success'
            }`}
      ...
      {/* Alerts */}
      {budgets.filter(b => getStatus(getPercentage(b.spent, b.amount)) === 'exceeded').length > 0 && (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-destructive/80">
                {budgets.filter(b => getStatus(getPercentage(b.spent, b.amount)) === 'exceeded').map(b => b.category).join(', ')}
              </p>
            </div>
          </div>
        </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground capitalize">{budget.period === 'monthly' ? 'Miesięczny' : budget.period === 'weekly' ? 'Tygodniowy' : 'Roczny'}</p>
                    </div>
                  </div>
                  <button
